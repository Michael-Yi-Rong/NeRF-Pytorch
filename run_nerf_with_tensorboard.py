...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if gt_imgs is not None:
            ...
            # 计算PSNR
            current_psnr = psnr(gt_img, render_img, data_range=1.0)
            psnrs.append(current_psnr)
            print(f"Frame {i} PSNR: {current_psnr:.2f} dB")
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_print==0:
            ...
            # 使用 PyTorch SummaryWriter
            writer.add_scalar('loss', loss.item(), global_step)
            writer.add_scalar('psnr', psnr.item(), global_step)
            writer.add_histogram('trans', trans.detach().cpu().numpy(), global_step)
            if args.N_importance > 0:
            ...
            #
            #     writer.add_image('rgb', to8b(rgb), global_step, dataformats='HWC')
            #     writer.add_image('disp', disp[..., None], global_step, dataformats='HWC')
            #     writer.add_image('acc', acc[..., None], global_step, dataformats='HWC')
            #     writer.add_scalar('psnr_holdout', psnr_hold.item(), global_step)
            ...
            #     if args.N_importance > 0:
            #
            #         writer.add_image('rgb0', to8b(extras['rgb0']), global_step, dataformats='HWC')
            #         writer.add_image('disp0', extras['disp0'][..., None], global_step, dataformats='HWC')
            #         writer.add_image('z_std', extras['z_std'][..., None], global_step, dataformats='HWC')
            #
            #         # with tf.contrib.summary.record_summaries_every_n_global_steps(args.i_img):
